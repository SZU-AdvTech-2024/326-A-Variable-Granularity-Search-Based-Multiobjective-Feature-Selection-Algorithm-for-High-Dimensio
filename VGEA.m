 function [pfs, errTr, selFeatNum] = VGEA(trainData, trainLabel, testData, testLabel, dataName, fold)
	%% Init
	featNum = size(trainData, 2);
	...
	% group
	[info, r] = SortFeature(trainData, trainLabel);     %info 是经过排序后的特征数据，r 是排序后特征的索引。
	idx = kmeans(info, groupNum);    %使用 K-Means 算法对特征数据进行聚类，idx 是聚类结果，它包含每个特征所属的组别索引。
	idx = idx';
	for i = 1 : groupNum
		g{i} = find(idx == i)';
	end
	...
	x = group2x(g, pop, featNum);       %group2x 函数的作用是将分组信息 g 和二进制矩阵 pop 转换为二进制编码 x，用于表示个体的特征选择情况。featNum 是特征数。
	objs = CalcObjs(trainData, trainLabel, testData, testLabel, x);
	[archive, archiveObjs] = UpdateArchive(x, objs, archive, archiveObjs, archiveNum);    %更新存档中的个体和对应的目标函数值

%% Iteration
	it = 2;
	while it <= maxIt
		for it2 = 1 : maxIt2
			interPop = [pop; x2group(g, archive)];    %合并种群和archive中存储的个体
			MatingPool = randi(size(interPop, 1), 1, popNum);
			pop2 = GeneticOperator(interPop(MatingPool, :), MatingPool > size(pop, 1), GetProb(g, p));

			x = group2x(g, pop2, featNum);     %将种群 pop2 和分组信息 g 转换为二进制编码 x，表示个体的特征选择情况。
			objs2 = CalcObjs(trainData, trainLabel, testData, testLabel, x);
		...
		if it > firstStageEnd
			...
			[archive, archiveObjs] = NSGAII(x, objs, popNum, it, maxIt, trainData, trainLabel, testData, testLabel, dataName, fold, ...
			archive, archiveObjs, archiveNum, r);
			it = inf;
		else
			% split
			[pop, objs, g] = Split(pop, objs, g, popNum, trainData, trainLabel);
		end
	end
...
function [archive, archiveObjs] = NSGAII(pop, objs, popNum, it, maxIt, trainData, trainLabel, testData, testLabel, dataName, fold,...
	archive, archiveObjs, archiveNum, r)
	...
	while it <= maxIt
		...
		objs2 = CalcObjs(trainData, trainLabel, testData, testLabel, pop2);
